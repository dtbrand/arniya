    ...
    (function() {
        ...
        /* ── Load Retailer Profile & State (Real Database Hydration) ── */
        window.loadSavedRetailerData = function() {
            ...
            var name = user.name || 'B2B Retailer';
            ...
            if (hdrName && name !== 'B2B Retailer') hdrName.textContent = name;
            var sideName = document.getElementById('sideUserName');
            if (sideName && name !== 'B2B Retailer') sideName.textContent = name;
            ...
            if (profName && !profName.value) profName.value = name !== 'B2B Retailer' ? name : '';
        ...
        window.handleSaveWholesalerDetails = function(e) {
            ...
            .then(function(res) {
                ...
                if (res.success) {
                    ...
                    window.showWsToast(res.message || 'Retailer profile updated in live database!', 'success');
                } else {
        ...
        window.handleSaveAddress = function(e) {
            ...
            if (isDispatchVisible && !isMainVisible) {
                if (!mComp) mComp = currentUser.companyName || currentUser.company_name || currentUser.name || 'Retailer Partner';
            ...
            if (!isSame) {
                ...
                if (isDispatchVisible && (!sAddr || !sCity || !sPin)) {
                    window.showWsToast('Please enter godown address, city, and 6-digit PIN code for custom dispatch.', 'error');
                    return false;
                }

                customShipping = {
                    warehouse_name: sWarehouse || 'Primary Storefront Dispatch Hub',
            ...
            function updateUIAndClose(successMsg) {
                ...
                var bFullEl = document.getElementById('addrPreviewBillingFull');
                var bAttnEl = document.getElementById('addrPreviewBillingAttn');
                if (bCompEl) bCompEl.textContent = mComp || (user.name || 'Registered Retailer');
                if (bFullEl) {
                ...
                if (bAttnEl) bAttnEl.textContent = 'Attn: ' + (user.name || mComp || 'Authorized Buyer') + ' (+91 ' + (mPhone || user.phone || '917046363528') + ')';
                ...
                window.showWsToast(successMsg || 'Address details saved successfully!', 'success');
            }
        ...
        function getDynamicAnalyticsData(mode, range) {
            ...
            var catMap = {};
            orders.forEach(function(o) {
                var pName = o.productName || 'Retail Consignment';
            ...
            if (mode === 'sales') {
                cards = [
                    { label: "Active Restock Lots", val: totalOrders + " Lots", pill: totalOrders > 0 ? "Active" : "0 Active", isGold: true },
                    { label: "Store Units Procured", val: totalUnits + " Pcs", pill: totalUnits > 0 ? "100% QC Passed" : "0 Pcs", isGold: false },
                ...
                cards = [
                    { label: "Gross Retail Spend", val: "₹" + totalSpend.toLocaleString('en-IN'), pill: "Gross Volume", isGold: true },
                ...
                cards = [
                    { label: "Retail Account Tier", val: totalOrders >= 50 ? "Tier 2" : "Tier 1", pill: totalOrders + " / 50 Orders", isGold: true },
                    { label: "Total Store Orders", val: String(totalOrders), pill: pendingOrders + " In Progress", isGold: false },
                    { label: "Restocked Quantity", val: totalUnits + " Pcs", pill: "Consignments", isGold: false },
                    { label: "Total Retail Turnover", val: "₹" + totalSpend.toLocaleString('en-IN'), pill: "Gross Volume", isGold: true }
                ];
            }

            return {
                sub: totalOrders > 0 ? "Live Retail Sales, Store Inventory Mix & Dispatch Reliability" : "Zero active retail consignments recorded. Place stock orders from the live catalog.",
                cards: cards,
                chartTitle: mode === 'sales' ? "Store Restock Units (Pcs)" : (mode === 'revenue' ? "Retail Turnover Volume (₹)" : "Monthly Sales Trend"),
                barActive: 7,
                gauge: {
                    pct: gaugePct + "%",
                    offset: gaugeOffset,
                    badge: totalOrders > 0 ? "+10.4% vs Target" : "Target Tracking",
                    desc: totalOrders > 0 ? ("You achieved <strong>₹" + totalSpend.toLocaleString('en-IN') + "</strong> this cycle. Just <strong>₹" + targetRemaining.toLocaleString('en-IN') + "</strong> left to complete " + tierName + " quota!") : "You have not placed any orders yet. Place your first order to start target tracking!",
                    target: "₹2.50L",
                ...
                cats: cats,
                kpis: [
                    { label: "Avg. Consignment Value", num: "₹" + avgOrderVal.toLocaleString('en-IN'), sub: totalOrders > 0 ? "Verified Average" : "No orders placed" },
                    { label: "Store Restock TAT", num: totalOrders > 0 ? "1.8 Days" : "0 Days", sub: WS_ICONS.lightning + " Priority Surat Express" },
                    { label: "GST Input Tax Credit", num: "₹" + gstCredit.toLocaleString('en-IN'), sub: WS_ICONS.shield + " 100% GSTR-1 Matched" },
                    { label: "Store Sell-Through", num: totalOrders > 0 ? "100%" : "0%", sub: WS_ICONS.repeat + " " + totalOrders + " Consignments Recorded" }
                ],
        ...
        window.downloadWalletStatement = function() {
            ...
            rows.push([
                ...
                `"Verified Retail Credit Line Allocation"`,
                `"Credit"`,
            ...
            if (o.items && Array.isArray(o.items) && o.items.length > 0) {
                ...
                <div style="display:flex; gap:14px; align-items:center; background:#FFFFFF; border:1.5px solid var(--ws-border); border-radius:10px; padding:14px; margin-bottom:14px; box-shadow:0 1px 4px rgba(0,0,0,0.03);">
                    ...
                    <div style="flex:1; min-width:0;">
                        ...
                        <div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:6px;">
                            <span style="font-size:0.72rem; font-weight:600; background:#FAF8F4; padding:2px 6px; border-radius:4px; border:1px solid var(--ws-border); color:var(--ws-text-sub);">SKU: ${o.sku || 'LOT-RT'}</span>
        ...
        window.openBillInvoiceModal = function(o) {
            ...
            var buyerName = user.name || (o.customer_name || 'Valued Retail Partner');
            var buyerComp = user.companyName || user.company_name || user.name || 'Registered Retail Enterprise';
        ...
        function renderTrackingTab(orders, selectedId) {
            ...
            if (!list || list.length === 0) {
                if (headerBadge) {
                    ...
                    headerBadge.innerHTML = 'No Active Shipments';
                }
                heroContainer.innerHTML = `
                    <div style="text-align:center; padding:32px 20px;">
                        <div style="width:54px; height:54px; border-radius:50%; background:#FAF5E8; border:1px solid #D4AF37; display:flex; align-items:center; justify-content:center; margin:0 auto 14px;">
                            <svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="#8A681F" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <h4 style="font-size:1.05rem; font-weight:800; color:var(--ws-text-main); margin-bottom:6px;">No Active Consignments to Track</h4>
                        <p style="font-size:0.82rem; color:var(--ws-text-muted); max-width:400px; margin:0 auto 16px;">
                            Once you place a consignment order from the B2B catalog, live GPS courier tracking, AWB details, and step-by-step dispatch milestones will appear here in real-time.
                        </p>
                        <button class="ws-btn ws-btn-primary ws-btn-sm" onclick="switchWsTab('catalog')" style="margin:0 auto;">
                            Browse Retail Catalog &rsaquo;
                        </button>
                    </div>
                `;
                gridContainer.innerHTML = `
                    <div style="grid-column: 1 / -1; text-align:center; padding:24px; color:var(--ws-text-muted); font-size:0.85rem;">
                        No shipments available under current filter.
                    </div>
                `;
                return;
            }
        ...
        window.openEditMainAddressModal = function(addrId) {
            ...
            try {
                ...
                var user = userRaw ? JSON.parse(userRaw) : {};
                var b2b = window.b2bUser || {};
                var merged = Object.assign({}, b2b, user);
                var el = function(id) { return document.getElementById(id); };
                
                var comp = merged.companyName || merged.company_name || merged.business_name || merged.name || '';
                var gstin = merged.gstin || merged.gst_number || '';
                var addr = merged.address || merged.address_line1 || '';
                var city = merged.city || 'Surat';
                var state = merged.state || 'Gujarat';
                var pin = merged.pincode || '395002';
                var phone = merged.rawPhone || (merged.phone ? String(merged.phone).replace(/\D/g, '').slice(-10) : '');

                var domComp = document.getElementById('addrPreviewBillingComp');
                if (!comp && domComp) comp = domComp.textContent.trim();

                if (el('wsMainEditCompName'))      el('wsMainEditCompName').value      = comp;
                if (el('wsMainEditGstNumber'))    el('wsMainEditGstNumber').value     = gstin;
                if (el('wsMainEditAddress'))      el('wsMainEditAddress').value       = addr;
                if (el('wsMainEditCity'))         el('wsMainEditCity').value          = city;
                if (el('wsMainEditState'))        el('wsMainEditState').value         = state;
                if (el('wsMainEditPincode'))      el('wsMainEditPincode').value       = pin;
                if (el('wsMainEditContactPhone')) el('wsMainEditContactPhone').value = phone;
            } catch(e) {}
        ...
        window.handleSaveMainAddressForm = function(e) {
            ...
            var btn = document.getElementById('wsBtnSaveMainAddress') ||
                      (e && e.target && e.target.querySelector ? e.target.querySelector('button[type="submit"]') : null);
            var oldHtml = btn ? btn.innerHTML : '';
            if (btn) {
                btn.disabled = true;
                btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#111827" stroke-width="2.5" style="animation: wsSpin 1s linear infinite;"><circle cx="12" cy="12" r="10" stroke-opacity="0.25" stroke="currentColor"></circle><path d="M12 2a10 10 0 0 1 10 10" stroke="currentColor"></path></svg> <span>Saving Billing Address...</span>';
            }
            ...
            .then(function(res) {
                if (btn) {
                    btn.disabled = false;
                    btn.innerHTML = oldHtml || '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#111827" stroke-width="2.2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> <span>Save Billing Address</span>';
                }

                var userRaw = localStorage.getItem('dtbrands_user');
                var user = userRaw ? JSON.parse(userRaw) : {};
                if (comp) { user.companyName = comp; user.company_name = comp; }
                if (gstin) { user.gst_number = gstin; user.gstin = gstin; }
                if (addr) { user.address = addr; user.address_line1 = addr; }
                ...
                if (window.b2bUser) {
                    if (comp) { window.b2bUser.companyName = comp; window.b2bUser.company_name = comp; }
                    if (gstin) { window.b2bUser.gstin = gstin; window.b2bUser.gst_number = gstin; }
                    if (addr) { window.b2bUser.address = addr; window.b2bUser.address_line1 = addr; }
                    if (city) window.b2bUser.city = city;
                    if (state) window.b2bUser.state = state;
                    if (pin) window.b2bUser.pincode = pin;
                    if (phone) { window.b2bUser.rawPhone = phone; window.b2bUser.phone = phone; }
                }
                ...
                var bFullEl = document.getElementById('addrPreviewBillingFull');
                var bAttnEl = document.getElementById('addrPreviewBillingAttn');
                if (bCompEl) bCompEl.textContent = comp || (user.name || 'Registered Retailer');
                if (bFullEl) {
                    var gstTxt = gstin ? ' (GSTIN: <strong>' + gstin + '</strong>)' : '';
                    bFullEl.innerHTML = addr + '<br>' + city + ', ' + state + ' - ' + pin + gstTxt;
                }
                if (bAttnEl) bAttnEl.textContent = 'Attn: ' + (comp || user.name || 'Authorized Buyer') + ' (+91 ' + (phone || user.phone || '') + ')';
        ...
        orders.slice(0, 4).forEach(function(o) {
            ...
            html += `
                <div class="ws-search-item" style="padding:8px 12px; border-bottom:1px solid #F4EFE6; display:flex; justify-content:space-between; align-items:center; cursor:pointer;" onclick="if(typeof openWsOrderModal==='function') openWsOrderModal('${oId}'); closeMobileSearchOverlay(); if(document.getElementById('wsGlobalSearchResults')) document.getElementById('wsGlobalSearchResults').style.display='none';">
                    ...
                    <span style="font-size:0.70rem; font-weight:800; color:var(--ws-gold-primary); display:inline-flex; align-items:center; gap:3px;">View <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"></polyline></svg></span>
                </div>
            `;
        });
    }
        ...
        prods.slice(0, 4).forEach(function(p) {
            html += `
                <div class="ws-search-item" style="padding:8px 12px; border-bottom:1px solid #F4EFE6; display:flex; justify-content:space-between; align-items:center; cursor:pointer;" onclick="if(typeof openQuickOrderModal==='function') openQuickOrderModal(${p.id}); closeMobileSearchOverlay(); if(document.getElementById('wsGlobalSearchResults')) document.getElementById('wsGlobalSearchResults').style.display='none';">
                    ...
                    <span style="font-size:0.70rem; font-weight:800; color:#047857; display:inline-flex; align-items:center; gap:3px;">Order <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"></polyline></svg></span>
                </div>
            `;
        });
    }
