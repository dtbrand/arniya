...
$customer_id = isset($_GET['id']) ? htmlspecialchars($_GET['id']) : '1';
$custData = null;
...
$cName = $custData['name'] ?? 'Pooja Sharma';
$cPhone = $custData['phone'] ?? '+91 98110 29381';
$cEmail = $custData['email'] ?? 'pooja.sharma92@gmail.com';
$cStatus = strtolower($custData['status'] ?? 'active');
$cType = ucfirst($custData['type'] ?? 'Retail');
$cDate = !empty($custData['created_at']) ? date('M d, Y', strtotime($custData['created_at'])) : 'Jan 12, 2025';
$cleanPhone = preg_replace('/[^0-9]/', '', $cPhone);
if (strlen($cleanPhone) === 10) $cleanPhone = '91' . $cleanPhone;
$initials = strtoupper(substr($cName, 0, 1) . (strpos($cName, ' ') !== false ? substr(explode(' ', $cName)[1], 0, 1) : ''));
?>

<!-- ══ CUSTOMER IDENTITY CARD (LEFT DOSSIER COLUMN) ══ -->
<div class="dt-cust-sidebar-card">
    <div class="dt-cust-dossier-hero">
        <div class="dt-cust-dossier-avatar"><?= $initials ?: 'PS' ?></div>
        <div>
            <h2 class="dt-cust-dossier-name"><?= htmlspecialchars($cName) ?></h2>
            <div class="dt-cust-dossier-meta">
                <span class="dt-status-pill <?= $cStatus === 'active' ? 'active' : 'inactive' ?>" style="font-size:0.65rem;">● <?= ucfirst($cStatus) ?> Verified</span>
                <span class="dt-status-pill vip" style="font-size:0.65rem;">★ <?= htmlspecialchars($cType) ?></span>
            </div>
            <span class="dt-cust-id-badge" style="display:block; margin-top:4px;">#<?= htmlspecialchars($customer_id) ?> • Registered <?= $cDate ?></span>
        </div>
    </div>
    ...
    <div class="dt-cust-contact-actions">
        <div class="dt-cust-contact-item">
            ...
            <div style="display:flex; gap:4px;">
                <button type="button" class="dt-cust-contact-btn" onclick="copyToClipboard('<?= addslashes($cPhone) ?>', 'Phone Number')">Copy</button>
        ...
        <div class="dt-cust-contact-item">
            ...
            <div style="display:flex; gap:4px;">
                <button type="button" class="dt-cust-contact-btn" onclick="copyToClipboard('<?= addslashes($cEmail) ?>', 'Email')">Copy</button>
                <a href="mailto:<?= htmlspecialchars($cEmail) ?>" class="dt-cust-contact-btn">Email</a>
            </div>
        </div>

        <a href="https://wa.me/<?= htmlspecialchars($cleanPhone) ?>?text=Hello%20<?= urlencode($cName) ?>%2C%20welcome%20to%20DT%20Brand%27s" target="_blank" class="dt-btn dt-btn-emerald" style="width:100%;">
    ...
    <div style="display:flex; flex-direction:column; gap:6px;">
        <a href="/admin/customers/edit.php?id=<?= htmlspecialchars($customer_id) ?>" class="dt-btn dt-btn-pale" style="width:100%;">
        ...
        <button type="button" class="dt-btn dt-btn-pale" style="width:100%;" onclick="openCustomerStatusModal('<?= htmlspecialchars($customer_id) ?>', '<?= htmlspecialchars($cStatus) ?>')">
