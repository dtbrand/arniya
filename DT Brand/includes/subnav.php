...
<nav class="dt-attached-subnav" id="dtAttachedSubnav" aria-label="Attached categories navigation">
    <div class="dt-subnav-scroll-track" id="dtMainCatSliderTrack" role="tablist">
        <a href="/DT Brand/" class="dt-subnav-item <?= (empty($currentCat) || strtolower($currentCat) === 'all') && basename($_SERVER['PHP_SELF']) === 'index.php' ? 'active' : '' ?>">
        ...
        <a href="/DT Brand/shop.php" class="dt-subnav-item <?= (empty($currentCat) || strtolower($currentCat) === 'all') && basename($_SERVER['PHP_SELF']) === 'shop.php' ? 'active' : '' ?>">
        ...
        ?>
        <button type="button" class="dt-subnav-item main-cat-tab <?= $isCatActive ? 'active' : '' ?>" role="tab" data-cat="<?= htmlspecialchars($hCat) ?>" aria-selected="<?= $isCatActive ? 'true' : 'false' ?>" onclick="if(typeof window.filterByBanner==='function'){window.filterByBanner('<?= htmlspecialchars(addslashes($hCat)) ?>');}else{window.location.href='/DT Brand/shop.php?category=<?= urlencode($hCat) ?>';}"><?= htmlspecialchars($hCat) ?></button>
        <?php endforeach; ?>
        <a href="/DT Brand/wholesale.php" class="dt-subnav-item" style="color:#FFE699; font-weight:800;">
            <span>📦 Wholesale Lots</span>
        </a>
        <a href="/DT Brand/reseller.php" class="dt-subnav-item" style="color:#FFE699; font-weight:800;">
