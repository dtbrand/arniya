    ...
    window.shareCurrentProduct = function() {
        if (typeof window.openSmartShareModal === 'function') {
            var prodData = {
                ...
                gallery: <?= json_encode((array)($product['images'] ?? $product['gallery'] ?? [])) ?>,
                images: <?= json_encode((array)($product['images'] ?? $product['gallery'] ?? [])) ?>,
                video: <?= json_encode((string)($product['video'] ?? '')) ?>,
                videos: <?= json_encode((array)($product['videos'] ?? [])) ?>,
