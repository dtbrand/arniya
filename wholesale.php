<?php
/**
 * wholesaler.php — Luxury DT Brand's B2B Wholesaler Dashboard
...
if ($currentUser && !empty($currentUser['id'])) {
    ...
    if ($pdo !== null && !Database::isMockMode()) {
        try {
            ...
            error_log('Wholesale orders loading error: ' . $e->getMessage());
        }
    }
}
...
foreach (ProductCatalog::getAll() as $dp) {
    ...
    // Use centralized price resolver for Wholesaler role
    $priceDisplay = ProductCatalog::getPriceDisplay($dp, 'wholesale');
    $wsPrice = $priceDisplay['effective_price'];
    $custPrice = (float)($dp['effective_customer_price'] ?? ($dp['customer_price'] ?? $wsPrice));
    ...
    $catalogProducts[] = [
        ...
        'retail_price'    => $wsPrice,
        'trade_price'     => $wsPrice,
        'customer_price'  => $custPrice,
        'wholesale_price' => $wsPrice,
        'reseller_price'  => (float)($dp['effective_reseller_price'] ?? $wsPrice),
        'price'           => $wsPrice,
...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/wholesale.css?v=<?= @filemtime(__DIR__ . '/assets/css/wholesale.css') ?: '3.2.1' ?>">
...
<body>
    ...
    <div class="ws-app-layout">
        ...
        <aside class="ws-sidebar" id="wsSidebar">
            <div class="ws-sidebar-brand-box">
                <a href="/wholesale.php" style="display:flex; align-items:center; text-decoration:none;">
                ...
                <div class="ws-side-user-card" onclick="switchWsTab('details'); toggleSidebar(false);" style="margin: 8px 8px 4px; padding: 7px 9px; background: linear-gradient(135deg, #FAF5E8 0%, #FFFFFF 100%); border: 1.2px solid rgba(212,175,55,0.35); border-radius: 9px; display: flex; align-items: center; gap: 8px; cursor: pointer; box-shadow: 0 2px 6px rgba(138,104,31,0.06);">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="font-size: 0.60rem; font-weight: 700; color: #8A681F; display: flex; align-items: center; gap: 2px;">
                            <span><svg width="12" height="12" viewBox="0 0 24 24" fill="#D4AF37" stroke="#8A681F" stroke-width="2" style="vertical-align:-1px; margin-right:3px;"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>Verified Wholesaler</span>
                        </div>
                    </div>
                        ...
                        <div class="ws-chart-wrapper" id="salesChartMainWrapper">
                            <div class="ws-chart-svg-container">
                                ...
                                <svg class="ws-chart-svg" viewBox="0 0 600 170" preserveAspectRatio="none" style="padding-left: 34px;">
                                    ...
                                    <!-- Dynamic Sales Area Gradient Fill -->
            ...
            <section class="ws-tab-pane" id="tabPaneSupport">
                <div class="ws-card">
                    <div class="ws-card-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                        ...
                        <a href="https://api.whatsapp.com/send?phone=917046363528&text=Hi%20DT%20Brand%27s%2C%20I%20am%20a%20registered%20Wholesaler%20and%20require%20urgent%20support" target="_blank" class="ws-btn ws-btn-wa ws-btn-sm" style="display:inline-flex; align-items:center; gap:6px; font-weight:700;">
                ...
                <div style="overflow-y: auto; flex: 1; padding: 16px 20px; max-height: calc(90vh - 135px);">
                    <div class="ws-form-grid">
                        ...
                        <div class="ws-form-group">
                            ...
                            <input type="text" id="wsUserAddrRecipient" class="ws-input" required placeholder="e.g. JNK HUB or Storefront Delivery">
                        </div>
    ...
    <div class="ws-modal-overlay" id="wsPrintableAuditReportModal" role="dialog" aria-modal="true" onclick="if(event.target===this) closePrintableAuditReportModal();">
        <div class="ws-modal-box" style="max-width: 820px;">
            <div class="ws-modal-header">
                <h3 class="ws-modal-title">
                    <span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="vertical-align:-2px; margin-right:4px;"><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg>Wholesale B2B Sales & Procurement Audit Statement</span>
                </h3>
                ...
                <div class="ws-invoice-sheet">
                    <div class="ws-inv-header">
                        <div class="ws-inv-brand">
                            ...
                            <p><strong>DT Brand's Wholesale Procurement & Logistics Ledger</strong><br>
        ...
        <div class="ws-modal-box" style="max-width: 440px;">
            <div class="ws-gate-box">
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    <a href="/account.php?tab=login&role=wholesale" class="ws-btn ws-btn-primary" style="text-decoration:none; justify-content:center;">
                    ...
                    <a href="https://api.whatsapp.com/send?phone=917046363528&text=Namaste+DT+Brands%2C+I+want+to+activate+my+Wholesale+B2B+Partner+Account" target="_blank" class="ws-btn ws-btn-emerald" style="text-decoration:none; justify-content:center;">
                    ...
                    <a href="/account.php?tab=register&role=wholesale" class="ws-btn ws-btn-secondary" style="text-decoration:none; justify-content:center;">
                        Register as New Wholesaler
                    </a>
    ...
    window.b2bMonthlyData = <?= json_encode(array_map(static fn($m, $val, $qty) => [
        'm' => $m,
        'val' => '₹' . number_format($val),
        'qty' => number_format($qty) . ' Pcs',
        'raw_val' => $val,
        'raw_qty' => $qty
    ], $monthLabels, $monthlySales, $monthlyUnits), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;
    </script>
