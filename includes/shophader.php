...
/* ── Mobile Animated Header Menu Trigger (Clean Transparent & Ultra-Clear Visibility) ── */
...
/* 3-Bar Pure Crisp Vector Animation (Obsidian #111827 with Heritage Gold #8A681F Highlight) */
.mobile-menu-bars {
    display: flex;
    flex-direction: column;
    justify-content: space-between;
...
.mobile-menu-bars .bar-top {
    width: 100%;
    background: #111827;
}

.mobile-menu-bars .bar-mid {
    width: 65%;
    background: #8A681F;
}

.mobile-menu-bars .bar-bot {
    width: 85%;
    background: #111827;
}

.header-mobile-menu-btn:hover .mobile-menu-bars .bar,
.header-mobile-menu-btn:focus .mobile-menu-bars .bar,
.header-mobile-menu-btn:active .mobile-menu-bars .bar {
    background: #8A681F;
    width: 100%;
}
...
<header class="shop-header" id="shopHeader">
    ...
    <nav class="header-attached-subnav" id="headerAttachedSubnav" aria-label="Attached categories navigation">
        <div class="subnav-scroll-track" id="mainCatSliderTrack">
            <button class="subnav-item main-cat-tab <?= empty($selectedCategory) || strtolower($selectedCategory) === 'all' ? 'active' : '' ?>" data-cat="All" <?= empty($selectedCategory) || strtolower($selectedCategory) === 'all' ? 'aria-current="page"' : '' ?> onclick="<?= $isHomePage ? 'window.location.href=\'/shop.php?category=all\';' : 'if(typeof window.filterByBanner===\'function\'){window.filterByBanner(\'All\');}else{window.location.href=\'/shop.php?category=all\';}' ?>">
            ...
            ?>
            <button class="subnav-item main-cat-tab <?= $isCatActive ? 'active' : '' ?>" data-cat="<?= htmlspecialchars($hCat) ?>" <?= $isCatActive ? 'aria-current="page"' : '' ?> onclick="<?= $isHomePage ? 'window.location.href=\'/shop.php?category=' . urlencode($hCat) . '\';' : 'if(typeof window.filterByBanner===\'function\'){window.filterByBanner(\'' . htmlspecialchars(addslashes($hCat)) . '\');}else{window.location.href=\'/shop.php?category=' . urlencode($hCat) . '\';}' ?>"><?= htmlspecialchars($hCat) ?></button>
            <?php endforeach; ?>
...
(function () {
    ...
    window.addEventListener('scroll', handleShopHeaderScroll, { passive: true });
    document.addEventListener('scroll', handleShopHeaderScroll, { passive: true });
    window.addEventListener('touchmove', handleShopHeaderScroll, { passive: true });
    window.addEventListener('resize', handleShopHeaderScroll, { passive: true });
    handleShopHeaderScroll();
