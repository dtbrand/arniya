 ...
 */
$customer_name = !empty($order['customer']) ? $order['customer'] : 'Direct Customer';
$customer_type = !empty($order['customer_type']) ? $order['customer_type'] : 'Customer';
$customer_phone = !empty($order['phone']) ? $order['phone'] : '+91 70463 63528';
$customer_email = !empty($order['email']) ? $order['email'] : '—';
$customer_orders = isset($order['customer_orders_count']) ? (int)$order['customer_orders_count'] : 1;
?>
    ...
    <div class="dt-detail-card-body" style="display:flex; flex-direction:column; gap:10px;">
        <div style="display:flex; align-items:center; gap:10px;">
            <div style="width:40px; height:40px; border-radius:50%; background:linear-gradient(135deg, #2A2010 0%, #443416 50%, #1C150B 100%); color:#FFE57F; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:14px; border:1.5px solid #D4AF37; box-shadow:0 2px 8px rgba(212,175,55,0.3); flex-shrink:0;">
                <?php echo strtoupper(substr(trim($customer_name), 0, 2)); ?>
            </div>
        ...
        <div style="display:flex; flex-direction:column; gap:6px; background:#FAF8F4; border:1px solid #E2DFD7; border-radius:6px; padding:10px; font-size:11.5px;">
            ...
            <div style="display:flex; justify-content:space-between;">
                ...
                <strong style="color:#8A681F;"><?php echo $customer_orders; ?> Recorded <?= $customer_orders === 1 ? 'Order' : 'Orders' ?></strong>
            </div>
        </div>
        ...
        <div style="display:flex; gap:6px;">
            ...
            <a href="/admin/orders/ledger.php?id=<?php echo urlencode($order['id'] ?? ($order_id ?? '')); ?>" onclick="if(window.DT_ORDER_VIEW && window.DT_ORDER_VIEW.openLedgerModal){ window.DT_ORDER_VIEW.openLedgerModal('<?php echo addslashes($customer_name); ?>', '<?php echo addslashes($customer_phone); ?>', '<?php echo addslashes($customer_email); ?>'); return false; }" target="_blank" class="dt-btn dt-btn-pale" style="flex:1; height:30px; font-size:11px;">
