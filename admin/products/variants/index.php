...
if ($pdo !== null && !Database::isMockMode()) {
    try {
        $stmt = $pdo->query("
            SELECT v.*, p.title as parent_name, p.primary_image, c.hex_code as color_hex
            FROM product_variants v
            LEFT JOIN products p ON v.product_id = p.id
            LEFT JOIN product_colors c ON v.color_id = c.id
            ORDER BY v.product_id DESC, v.id ASC
        ");
        $dbVariants = $stmt->fetchAll() ?: [];
        foreach ($dbVariants as $v) {
            $pPrice = (float)($v['price'] ?? 0);
            $pWholesale = (float)($v['wholesale_price'] ?? ($pPrice * 0.7));
            $pStock = (int)($v['stock_qty'] ?? 0);
            $variants_matrix[] = [
                'id' => (int)$v['id'],
                'parent_name' => $v['parent_name'] ?? 'Product #' . $v['product_id'],
                'parent_id' => (int)$v['product_id'],
                'sku' => $v['sku'] ?? ('VAR-' . $v['id']),
                'color' => $v['color_name'] ?? 'Standard',
                'color_hex' => $v['color_hex'] ?? '#8A681F',
                'size' => $v['size_name'] ?? 'Free Size (6.3m)',
                ...
                'img' => $v['image'] ?: ($v['primary_image'] ?: '/assets/images/product1.png')
            ];
        }
    } catch (\Throwable $e) {
        $variants_matrix = [];
            ...
            <div class="dt-kpi-ribbon">
                <div class="dt-kpi-card">
                    ...
                    <div>
                        ...
                        <div style="font-size:17px; font-weight:800; color:#181512;"><?php echo count($variants_matrix); ?> Combinations</div>
                    </div>
                </div>
                    ...
                    <tbody id="variantsTableBody">
                        ...
                        <tr style="border-bottom:1px solid #f0f0f1; transition:background 0.15s;" onmouseover="this.style.background='#FDFBF7'" onmouseout="this.style.background='transparent'">
                            ...
                            <td style="padding:10px 10px;">
                                ...
                                <?php else: ?>
                                <span class="adm-badge" style="background:#FEF2F2; color:#DC2626; font-weight:700; font-size:11px; padding:3px 8px; border-radius:12px; display:inline-flex; align-items:center; gap:4px;">
                                    <svg viewBox="0 0 24 24" width="10" height="10" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg>
                                    Low Stock
                                </span>
                                <?php endif; ?>
