...
if ($currentUser && !empty($currentUser['id'])) {
    ...
    if ($pdo !== null && !Database::isMockMode()) {
        try {
            ...
            $spend = 0.0;
            $pending = 0;
            $commRate = (float)($dbUser['commission_rate'] ?? 15.0);
            ...
            $realKpis['total_earnings'] = round($spend * ($commRate / 100), 2);
            $realKpis['pending_orders'] = $pending;
            $realKpis['tier'] = !empty($dbUser['tier']) ? $dbUser['tier'] : 'Verified Reseller Partner';
            $realKpis['commission_rate'] = $commRate;
        } catch (\Throwable $e) {
            error_log('Reseller orders loading error: ' . $e->getMessage());
        }
    }
}

$activeUserName = $dbUser['name'] ?? ($currentUser['name'] ?? 'Reseller Partner');
...
if (empty($activeUserEmail) || preg_match('/^\d+$/', $activeUserEmail)) {
    $activeUserEmail = !empty($activeUserPhoneDigits) ? ($activeUserPhoneDigits . '@dtbrands.in') : 'reseller@dtbrands.in';
}
...
$activeUserAddress = !empty($dbUser['address']) ? $dbUser['address'] : (!empty($dbUser['address_line1']) ? $dbUser['address_line1'] : 'Commercial Reseller Hub Address');
...
// Real dynamic KPI and order variables for reseller
$totalOrders = count($realOrders);
$lifetimeSpend = (float)$realKpis['lifetime_spend'];
$totalEarnings = (float)$realKpis['total_earnings'];
$pendingOrders = (int)$realKpis['pending_orders'];
$creditLimit = (float)($dbUser['credit_limit'] ?? 0);
$outstandingBalance = (float)($dbUser['outstanding_balance'] ?? 0);
...
$processingCount = 0;
foreach ($realOrders as $ro) {
    $totalUnits += (int)($ro['items_count'] ?? 1);
    $st = strtolower((string)($ro['status'] ?? ''));
    if (in_array($st, ['delivered', 'completed'], true)) {
        $deliveredCount++;
    } elseif (in_array($st, ['shipped', 'in_transit', 'in transit', 'dispatched'], true)) {
        $shippedCount++;
    } else {
        $processingCount++;
    }
...
$tierName = $totalOrders >= 50 ? 'Tier 2: Silver Partner' : 'Tier 1: Active Reseller';
$tierRemaining = max(0, 50 - $totalOrders);
$targetQuota = 250000;
$targetAchieved = $lifetimeSpend;
$targetRemaining = max(0, $targetQuota - $targetAchieved);
$gaugePercent = $targetQuota > 0 ? min(100, round(($targetAchieved / $targetQuota) * 100, 2)) : 0;
...
$gaugeRad = M_PI * (1.0 - ($gaugePercent / 100.0));
...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/reseller.css?v=<?= @filemtime(__DIR__ . '/assets/css/reseller.css') ?: '3.2.1' ?>">
    ...
    <script>
        ...
        window.b2bUser = <?php echo json_encode([
            'id' => $currentUser['id'] ?? null,
            'name' => $activeUserName,
            'email' => $activeUserEmail,
            'phone' => $activeUserPhone,
            'rawPhone' => preg_replace('/[^0-9]/', '', $activeUserPhone),
            'companyName' => $activeUserCompanyName,
            'company_name' => $activeUserCompanyName,
            'business_name' => $activeUserCompanyName,
            ...
            'tier' => $activeUserTier,
            'total_earnings' => $realKpis['total_earnings'] ?? 0.0,
            'commission_rate' => $realKpis['commission_rate'] ?? 15.0
...
<body>
    ...
    <div class="ws-app-layout">
        ...
        <aside class="ws-sidebar" id="wsSidebar">
            <div class="ws-sidebar-brand-box">
                <a href="/reseller.php" style="display:flex; align-items:center; text-decoration:none;">
            ...
            <div class="ws-sidebar-scroll">
                <!-- VIP Reseller User Card in Sidebar -->
                <div class="ws-side-user-card" onclick="switchWsTab('details'); toggleSidebar(false);" style="margin: 8px 8px 4px; padding: 7px 9px; background: linear-gradient(135deg, #FAF5E8 0%, #FFFFFF 100%); border: 1.2px solid rgba(212,175,55,0.35); border-radius: 9px; display: flex; align-items: center; gap: 8px; cursor: pointer; box-shadow: 0 2px 6px rgba(138,104,31,0.06);">
                    ...
                    <div style="flex: 1; min-width: 0;">
                        ...
                        <div style="font-size: 0.60rem; font-weight: 700; color: #8A681F; display: flex; align-items: center; gap: 2px;">
                            <span><svg width="12" height="12" viewBox="0 0 24 24" fill="#D4AF37" stroke="#8A681F" stroke-width="2" style="vertical-align:-1px; margin-right:3px;"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>Verified Reseller</span>
                        </div>
                    </div>
                ...
                <div class="ws-nav-category">RESELLER CRM & PROFIT</div>
                <ul class="ws-nav-list">
                    <li>
                        <a class="ws-nav-item" onclick="switchWsTab('customers')">
                            <svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                            <span>Customers</span>
                            <span class="ws-nav-badge gold" id="navCustomersCount">12</span>
                        </a>
                    </li>
                    <li>
                        <a class="ws-nav-item" onclick="openResellerQuickOrderDrawer()">
                            <svg viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>
                            <span>Quick Order</span>
                            <span class="ws-nav-badge" style="background:#10B981; color:#FFFFFF;">FAST</span>
                        </a>
                    </li>
                    <li>
                    ...
                    <li>
                        <a class="ws-nav-item" onclick="switchWsTab('followups')">
                            <svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                            <span>Customers Follow-up</span>
                            <span class="ws-nav-badge" id="navFollowupsDueBadge" style="background:#EF4444; color:#FFFFFF; display:none;">0</span>
                        </a>
                    </li>
                    <li>
                        <a class="ws-nav-item" onclick="switchWsTab('recommendations')">
                            <svg viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
        ...
        <main class="ws-main-content">
                ...
                <div class="ws-metric-cards-grid">
                    ...
                    <div class="ws-stat-box" onclick="openVipTierModal()" style="cursor:pointer; position:relative;" title="Tap to view VIP Tier Roadmap">
                        ...
                        <div class="ws-tier-ribbon-tag non-vip" id="wsTierRibbonTag">
                            <span id="wsTierRibbonText"><?= $totalOrders >= 50 ? 'TIER 2 VIP' : 'NON VIP' ?></span>
                        </div>
                    ...
                    <div class="ws-stat-box">
                        ...
                        <div class="ws-stat-val-row">
                            <div class="ws-stat-val-num" id="statVal2"><?= $totalOrders ?></div>
                            <span class="ws-trend-pill up" id="statPill2"><?= $pendingOrders ?> In Progress</span>
                        </div>
                    </div>
                    ...
                    <div class="ws-stat-box">
                        ...
                        <div class="ws-stat-val-row">
                            ...
                            <span class="ws-trend-pill up" id="statPill3"><?= $totalOrders ?> Consignments</span>
                        </div>
                    </div>
                ...
                <div class="ws-analytics-grid">
                    ...
                    <div class="ws-analytics-card">
                        ...
                        <div class="ws-chart-tooltip" id="chartLiveTooltip">
                            ...
                            <span id="chartTooltipText">Aug (Current): ₹<?= number_format($lifetimeSpend) ?> • <?= number_format($totalUnits) ?> Pcs</span>
                        </div>
                    ...
                    <div class="ws-analytics-card">
                        <div class="ws-card-top-bar" style="margin-bottom: 8px;">
                            <div>
                                <h3 style="margin:0; font-family:var(--ws-font-serif); font-size:1.02rem; color:var(--ws-gold-primary); font-weight:800;">Reseller Target</h3>
                                <p style="font-size:0.72rem; color:var(--ws-text-muted); margin-top:2px;">Monthly resale target & profit velocity</p>
                            </div>
                        ...
                        <div class="ws-gauge-wrap">
                            <svg class="ws-gauge-svg" viewBox="0 0 200 110">
                                ...
                                <circle id="targetGaugeIndicatorGlow" cx="<?= $gaugeTipCx ?>" cy="<?= $gaugeTipCy ?>" r="6" fill="#FFE082" stroke="#8A681F" stroke-width="2.5" filter="url(#gaugeGlow)" style="<?= $gaugePercent > 0 ? '' : 'opacity:0;' ?>" />
                                <circle id="targetGaugeIndicatorDot" cx="<?= $gaugeTipCx ?>" cy="<?= $gaugeTipCy ?>" r="2.5" fill="#FFFFFF" style="<?= $gaugePercent > 0 ? '' : 'opacity:0;' ?>" />
                            </svg>
                            ...
                            <p class="ws-gauge-desc" id="targetGaugeDesc">
                                <?= $totalOrders > 0 ? ('You achieved <strong>₹' . number_format($targetAchieved) . '</strong> in resale orders. Just <strong>₹' . number_format($targetRemaining) . '</strong> left to complete ' . htmlspecialchars($tierName) . ' quota!') : 'You have not placed any orders yet. Place your first order to start target tracking!' ?>
                            </p>
                        </div>
                ...
                <div class="ws-statistics-card">
                    <div class="ws-section-header" style="margin-bottom:0;">
                        <div class="ws-sec-title-box">
                            ...
                            <p id="statsSubtitle"><?= $totalOrders > 0 ? "Weekly Reseller Sales, Profit Margins & Customer Delivery Performance" : "Zero active reseller consignments recorded. Place stock orders from the live catalog." ?></p>
                        </div>
                        ...
                        <div class="ws-sec-actions-row">
                            ...
                            <div class="ws-date-range-pill" id="dateRangeBtn" onclick="openDateRangePicker()">
                                ...
                                <span id="selectedDateRangeLabel">Current Cycle</span>
                    ...
                    <div class="ws-stats-grid" id="statsDynamicContent">
                        ...
                        <div>
                            <div class="ws-stats-col-title">
                                <span id="catBreakdownTitle"><svg class="ws-ico gold" style="margin-right:5px;" viewBox="0 0 24 24"><path d="M20.38 3.46L16 2 12 5.5 8 2l-4.38 1.46a2 2 0 0 0-1.34 2.23l.58 3.47a1 1 0 0 0 .99.84H6v10a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V10h2.15a1 1 0 0 0 .99-.84l.58-3.47a2 2 0 0 0-1.34-2.23z"></path></svg> Category Resale Breakdown</span>
                            </div>
                            <div class="ws-cat-prog-list" id="catProgList">
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val"><?= $totalOrders > 0 ? ('₹' . number_format(round($lifetimeSpend * 0.56)) . ' (56%)') : '₹0 (0%)' ?></span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                        <div class="ws-cat-prog-fill" style="--prog-w: <?= $totalOrders > 0 ? '88%' : '0%' ?>; animation-delay: 0.6s;"></div>
                                    </div>
                                </div>
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val"><?= $totalOrders > 0 ? ('₹' . number_format(round($lifetimeSpend * 0.24)) . ' (24%)') : '₹0 (0%)' ?></span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                        <div class="ws-cat-prog-fill" style="--prog-w: <?= $totalOrders > 0 ? '72%' : '0%' ?>; animation-delay: 0.75s;"></div>
                                    </div>
                                </div>
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val"><?= $totalOrders > 0 ? ('₹' . number_format(round($lifetimeSpend * 0.13)) . ' (13%)') : '₹0 (0%)' ?></span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                        <div class="ws-cat-prog-fill" style="width: <?= $totalOrders > 0 ? '95%' : '0%' ?>;"></div>
                                    </div>
                                </div>
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val"><?= $totalOrders > 0 ? ('₹' . number_format(round($lifetimeSpend * 0.07)) . ' (7%)') : '₹0 (0%)' ?></span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                        <div class="ws-cat-prog-fill" style="width: <?= $totalOrders > 0 ? '60%' : '0%' ?>;"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="ws-kpi-grid" id="kpiGrid">
                                ...
                                <div class="ws-kpi-box">
                                    ...
                                    <div class="ws-kpi-sub"><svg class="ws-ico gold ws-ico-sm" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg> Express Customer Dispatch</div>
                                </div>
                                <div class="ws-kpi-box">
                                    <div class="ws-kpi-label">Total Resale Earned</div>
                                    <div class="ws-kpi-num">₹<?= number_format($totalEarnings) ?></div>
                                    <div class="ws-kpi-sub"><svg class="ws-ico gold ws-ico-sm" viewBox="0 0 24 24"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg> <?= $totalOrders ?> Orders Realized</div>
                                </div>
                                <div class="ws-kpi-box">
                                    <div class="ws-kpi-label">Customer Repeat Rate</div>
                                    <div class="ws-kpi-num"><?= $totalOrders > 0 ? '88.5%' : '0%' ?></div>
                                    <div class="ws-kpi-sub"><svg class="ws-ico gold ws-ico-sm" viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg> 5.0 Buyer Rating</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="ws-tier-status-box" style="padding:14px; display:flex; flex-direction:column; justify-content:space-between; background:linear-gradient(135deg, #FFFFFF 0%, #FEFAF0 100%); border:1.5px solid rgba(212,175,55,0.45); border-radius:12px;">
                                <div>
                                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                                        <span class="ws-tier-badge active" id="statsMilestoneBadge" style="font-size:0.60rem; font-weight:800;"><?= $tierName ?></span>
                                    ...
                                    <p id="statsMilestoneDesc" style="font-size:0.73rem; color:#57534E; line-height:1.35; margin:0;">
                                        Complete <strong><?= $tierRemaining ?> more orders</strong> to automatically unlock <strong>Tier 2: Silver Reseller</strong> with an extra +3% margin rebate!
                                    </p>
                                </div>
                    ...
                    <div class="ws-slider-wrap">
                        ...
                        <div class="ws-slider-track" id="wsTrendingSliderTrack">
                            ...
                            <article class="product-card" data-product-id="<?= $prod['id'] ?>" role="listitem">
                                ...
                                <div class="card-body">
                                    <h2 class="card-name">
                                        <a href="/product.php?id=<?= $prod['id'] ?>" style="color:inherit;text-decoration:none;">
            ...
            ══�                        <div class="ws-form-grid">
                            ...
                            <div class="ws-form-group">
                                ...
                                <input type="text" class="ws-input" value="Reseller B2B VIP Tier" disabled>
                            </div>
            ...
            <section class="ws-tab-pane" id="tabPaneReports">
                ...
                <div class="ws-report-kpis-grid">
                    ...
                    <div class="ws-kpi-box">
                        ...
                        <div class="ws-kpi-sub"><?= $totalOrders ?> Reseller Consignments</div>
                    </div>
                    <div class="ws-kpi-box">
                ...
                <div class="ws-card">
                    <div class="ws-card-header" style="flex-wrap:wrap; gap:12px; align-items:center;">
                        <div class="ws-card-title-group">
                            <h3>B2B Reseller Procurement Reports</h3>
                        </div>
                        <div style="display:flex; gap:8px; flex-wrap:wrap;">
                            <button class="ws-btn ws-btn-secondary ws-btn-sm" onclick="exportReportsToCsv()">
                                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>Export CSV
                            </button>
            ...
            <section class="ws-tab-pane" id="tabPaneSupport">
                <div class="ws-card">
                    <div class="ws-card-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                        ...
                        <a href="https://api.whatsapp.com/send?phone=917046363528&text=Hi%20DT%20Brand%27s%2C%20I%20am%20a%20registered%20Reseller%20and%20require%20urgent%20support" target="_blank" class="ws-btn ws-btn-wa ws-btn-sm" style="display:inline-flex; align-items:center; gap:6px; font-weight:700;">
                    ...
                    <div class="ws-card-header">
                        <div class="ws-card-title-group">
                            <h2 class="ws-card-title" style="font-family:var(--ws-font-serif); font-size:1.15rem; font-weight:900; margin:0; color:var(--ws-gold-primary);">
                                <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="var(--ws-gold-primary)" stroke-width="2.2" style="display:inline-block; vertical-align:middle; margin-right:6px;"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>Reseller Customers CRM Directory
                            </h2>
                        ...
                        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                            ...
                            <button class="ws-btn ws-btn-secondary ws-btn-sm" onclick="openSavedFiltersModal('customers')">
                                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px;"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>Saved Filters
                            </button>
                            <button class="ws-btn ws-btn-primary ws-btn-sm" onclick="openAddCustomerModal()">
                                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#FFFFFF" stroke-width="2.5" style="margin-right:4px;"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>Add New Customer
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="crm-kpi-card">
                            ...
                            <div class="crm-kpi-icon">
                                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="#8A681F" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M6 3h12M6 8h9M6 3v9.5c3.5 0 6-1 6-4.75S9.5 3 6 3M10.5 12.5L16.5 21"></path>
                                </svg>
                            </div>
                        </div>
                        <div class="crm-kpi-card">
                ...
                <div class="ws-card ws-rec-main-card">
                    ...
                    <div class="ws-rec-customer-box">
                        <div class="ws-rec-select-header">
                            ...
                            <span class="ws-rec-select-hint">Personalized WhatsApp Pitch</span>
                        </div>
        ...
        <div class="ws-modal-box" style="max-width: 620px;">
            <div class="ws-modal-header">
                <h3 class="ws-modal-title">
                    ...
                    <span id="modalOrderTitle">Reseller Order Details</span>
                </h3>
    ...
    <div class="ws-modal-overlay" id="wsPrintableAuditReportModal" role="dialog" aria-modal="true" onclick="if(event.target===this) closePrintableAuditReportModal();">
        <div class="ws-modal-box" style="max-width: 820px;">
            <div class="ws-modal-header">
                <h3 class="ws-modal-title">
                    <span><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="vertical-align:-2px; margin-right:4px;"><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg>Reseller B2B Sales & Procurement Audit Statement</span>
                </h3>
                ...
                <div style="border:1.5px solid var(--ws-border); border-radius:10px; overflow:hidden; background:#FFFFFF; box-shadow:0 2px 8px rgba(0,0,0,0.03);">
                    <?php if (empty($b2bOrders)): ?>
                        <div style="text-align:center; padding:24px 12px; font-size:0.82rem; color:var(--ws-text-muted);">
                            No wallet ledger deductions recorded yet. Available balance: ₹<?= number_format($availableBalance) ?>.
                        </div>
                    <?php else: ?>
        ...
        <div class="ws-modal-box" style="max-width: 440px;">
            <div class="ws-gate-box">
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    <a href="/account.php?tab=login&role=reseller" class="ws-btn ws-btn-primary" style="text-decoration:none; justify-content:center;">
                    ...
                    <a href="https://api.whatsapp.com/send?phone=917046363528&text=Namaste+DT+Brands%2C+I+want+to+activate+my+Reseller+B2B+Partner+Account" target="_blank" class="ws-btn ws-btn-emerald" style="text-decoration:none; justify-content:center;">
                        ...
                        <span>1-Click WhatsApp B2B Verification</span>
                    </a>
                    <a href="/account.php?tab=register&role=reseller" class="ws-btn ws-btn-secondary" style="text-decoration:none; justify-content:center;">
    ...
    ═══════════════════════════════════════════ -->
    <div class="ws-modal-overlay" id="resellerCustomerProfileModal" role="dialog" aria-modal="true" onclick="if(event.target===this) closeCustomerProfileModal();">
        <div class="ws-modal-box" style="max-width: 900px; max-height: 90vh; display: flex; flex-direction: column;">
            <div class="ws-modal-header" style="background:#FAF8F4; border-bottom:1px solid var(--ws-border);">
                <h3 class="ws-modal-title" style="display:flex; align-items:center; gap:8px;">
                    <span>Customer Profile & CRM 360°</span>
                </h3>
                <button class="ws-modal-close-btn" onclick="closeCustomerProfileModal()">&times;</button>
            </div>
            
            <div class="ws-modal-body" style="padding: 16px; overflow-y: auto;">
                <!-- Profile Hero -->
                <div class="crm-profile-hero" id="crmProfileHero">
                    <!-- Injected dynamically by JS -->
                </div>

                <!-- 1-Tap Action Ribbon -->
                <div class="crm-action-ribbon" id="crmProfileActionRibbon" style="margin-bottom: 16px;">
                    <!-- Injected dynamically by JS -->
                </div>

                <!-- Navigation Tabs -->
                <div class="crm-profile-nav" id="crmProfileNav">
                    <button class="crm-profile-tab active" onclick="switchProfileTab('orders', this)">Order History</button>
                    <button class="crm-profile-tab" onclick="switchProfileTab('products', this)">Purchased Products</button>
                ...
                <div id="profTab-orders" class="crm-prof-tab-content">
                    <div id="profOrdersList" style="display:flex; flex-direction:column; gap:8px;"></div>
                </div>

                <div id="profTab-products" class="crm-prof-tab-content" style="display:none;">
                    <div id="profProductsList" class="ws-catalog-grid" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr));"></div>
                </div>

                <div id="profTab-recommended" class="crm-prof-tab-content" style="display:none;">
                    <div id="profRecommendedList" class="ws-catalog-grid" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr));"></div>
                </div>

                <div id="profTab-ledger" class="crm-prof-tab-content" style="display:none;">
                    <div id="profLedgerContent"></div>
                </div>

                <div id="profTab-notes" class="crm-prof-tab-content" style="display:none;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <span style="font-size:0.80rem; font-weight:800;">Customer Notes History</span>
                        <button class="ws-btn ws-btn-primary ws-btn-sm" onclick="openAddNoteModal(currentActiveProfileCustomer.id)">+ Add Note</button>
                    </div>
                    <div id="profNotesList" style="display:flex; flex-direction:column; gap:8px;"></div>
                </div>

                <div id="profTab-followups" class="crm-prof-tab-content" style="display:none;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <span style="font-size:0.80rem; font-weight:800;">Customer Follow-up Tasks</span>
                        <button class="ws-btn ws-btn-primary ws-btn-sm" onclick="openScheduleFollowupModal(currentActiveProfileCustomer.id)">+ Add Follow-up</button>
                    </div>
                    <div id="profFollowupsList" style="display:flex; flex-direction:column; gap:8px;"></div>
                </div>

                <div id="profTab-timeline" class="crm-prof-tab-content" style="display:none;">
                    <div class="crm-timeline" id="profTimelineList"></div>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="ws-modal-luxury-body">
                    ...
                    <div class="ws-smart-form-section">
                        ...
                        <div class="ws-smart-chips-wrap" id="custSmartTagChips">
                            ...
                            <span class="ws-smart-chip" onclick="toggleCustTagChip(this, 'RETAIL')">Retail</span>
                ...
                <div class="ws-modal-luxury-title-wrap">
                    ...
                    <div>
                        <h3 class="ws-modal-luxury-title">
                            <span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="vertical-align:-2px; margin-right:4px;"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"></path></svg>Reseller Quick Order Flow</span>
                        </h3>
                            ...
                            <div class="ws-qo-selected-cust-left">
                                ...
                                <div>
                                    <div class="ws-qo-cust-name-row">
                                        ...
                                        <span class="ws-qo-cust-phone" id="qoSelectedCustPhone">7046363528</span>
                                    </div>
                    ...
                    <div class="ws-smart-form-section">
                        ...
                        <div class="ws-qo-prod-search-row" id="qoProdSearchRow">
                            <div class="ws-qo-search-wrap" style="flex:1;">
                                ...
                                <input type="text" id="qoProductSearchInput" class="ws-qo-search-input" placeholder="Search product by SKU, name, category or fabric..." autocomplete="off" oninput="handleQoProductSearchInput(this.value)" onfocus="handleQoProductSearchFocus()">
                        ...
                        <div class="ws-qo-selected-prod-card" id="qoSelectedProductCard" style="display:none;">
                            <div class="ws-qo-selected-prod-left">
                                <!-- These placeholders showed a real-looking product (product1.png,
                                     "Pure Kanjivaram Silk Saree", SKU JHT-101, cost Rs 1,399 /
                                     MRP Rs 3,499). The card is only ever shown after a product is
                                     picked, so it now starts blank and JS fills in the real row. -->
                                <img src="/assets/images/no-image.svg" alt="" class="ws-qo-prod-img" id="qoSelectedProdImg">
                                <div>
                                    <div class="ws-qo-prod-name-row">
                                        <span class="ws-qo-prod-name" id="qoSelectedProdName"></span>
                                        <span class="ws-qo-prod-sku" id="qoSelectedProdSku"></span>
                                    </div>
                                    <div class="ws-qo-prod-pricing-snippet" id="qoSelectedProdPricing">
                                        Cost: <strong id="qoSelectedProdCost">—</strong> &bull; MRP: <strong id="qoSelectedProdMrp">—</strong>
                                    </div>
                                </div>
                            </div>
