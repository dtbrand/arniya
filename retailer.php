...
// Dynamic real order calculations
$totalOrders = count($realOrders);
$lifetimeSpend = 0.0;
$pendingOrders = 0;
$creditLimit = (float)($realKpis['credit_limit'] ?? 0);
$outstandingBalance = (float)($realKpis['outstanding_balance'] ?? 0);
$availableBalance = max(0.0, $creditLimit - $outstandingBalance);
$totalCoins = 0;
$totalUnits = 0;
$shippedCount = 0;
$deliveredCount = 0;
...
foreach ($realOrders as $idx => $ro) {
    $amt = (float)($ro['total_amount'] ?? $ro['total'] ?? 0);
    $lifetimeSpend += $amt;
    $qty = (int)($ro['items_count'] ?? $ro['qty'] ?? 1);
    $totalUnits += $qty;
    $st = strtolower((string)($ro['status'] ?? ''));
    if (in_array($st, ['shipped', 'in_transit', 'in transit', 'dispatched'], true)) {
        $shippedCount++;
...
$tierName = $totalOrders >= 50 ? 'Tier 2 (Silver)' : 'Tier 1 (1–50 Orders)';
$tierRemaining = max(0, 50 - $totalOrders);
$targetQuota = 250000.0;
$targetAchieved = $lifetimeSpend;
$targetRemaining = max(0.0, $targetQuota - $targetAchieved);
$targetPercent = $targetQuota > 0 ? min(100.0, round(($targetAchieved / $targetQuota) * 100, 2)) : 0.0;
$gaugePercent = number_format($targetPercent, 2);
...
if (empty($activeUserEmail) || preg_match('/^\d+$/', $activeUserEmail)) {
    $activeUserEmail = !empty($activeUserPhoneDigits) ? ($activeUserPhoneDigits . '@dtbrands.in') : 'retailer@dtbrands.in';
}
...
$activeUserAddress = !empty($dbUser['address']) ? $dbUser['address'] : (!empty($dbUser['address_line1']) ? $dbUser['address_line1'] : 'Commercial Retail Storefront Address');
...
foreach (ProductCatalog::getAll() as $dp) {
    ...
    foreach (['single' => 'Single', 'half_set' => 'Half set', 'full_set' => 'Full set', 'master_bale' => 'Master bale'] as $lotKey => $lotLabel) {
        ...
        if ($lotQty > 0) {
            $lotParts[] = $lotLabel . ': ' . $lotQty . ' pcs' . ($tradePrice > 0 ? ' @ Rs ' . number_format($tradePrice) . '/pc' : '');
        }
    }
...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/retailer.css?v=<?= @filemtime(__DIR__ . '/assets/css/retailer.css') ?: '3.2.1' ?>">
...
<body>
    ...
    <div class="ws-app-layout">
        ...
        <aside class="ws-sidebar" id="wsSidebar">
            <div class="ws-sidebar-brand-box">
                <a href="/retailer.php" style="display:flex; align-items:center; text-decoration:none;">
                ...
                <div class="ws-analytics-grid">
                    ...
                    <div class="ws-analytics-card">
                        ...
                        <div class="ws-chart-tooltip" id="chartLiveTooltip">
                            ...
                            <span id="chartTooltipText">Live Cycle: ₹<?= number_format($lifetimeSpend) ?> • <?= number_format($totalUnits) ?> Pcs</span>
                        </div>
                    ...
                    <div class="ws-analytics-card">
                        ...
                        <div class="ws-gauge-stats-row">
                            ...
                            <div class="ws-gauge-stat-pill">
                                ...
                                <div class="ws-g-stat-val" id="gStatRevenue" style="color:var(--ws-success); display:inline-flex; align-items:center; gap:4px;">₹<?= number_format($lifetimeSpend) ?> <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline><polyline points="17 6 23 6 23 12"></polyline></svg></div>
                            </div>
                            <div class="ws-gauge-stat-pill">
                    ...
                    <div class="ws-section-header" style="margin-bottom:0;">
                        ...
                        <div class="ws-sec-actions-row">
                            ...
                            <div class="ws-date-range-pill" id="dateRangeBtn" onclick="openDateRangePicker()">
                                ...
                                <span id="selectedDateRangeLabel">Live Cycle</span>
                            ...
                            <div class="ws-cat-prog-list" id="catProgList">
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val">₹<?= number_format(round($lifetimeSpend * 0.6)) ?> (60%)</span>
                                    </div>
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val">₹<?= number_format(round($lifetimeSpend * 0.25)) ?> (25%)</span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val">₹<?= number_format(round($lifetimeSpend * 0.10)) ?> (10%)</span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                                ...
                                <div class="ws-cat-prog-item">
                                    <div class="ws-cat-prog-header">
                                        ...
                                        <span class="ws-cat-prog-val">₹<?= number_format(round($lifetimeSpend * 0.05)) ?> (5%)</span>
                                    </div>
                                    <div class="ws-cat-prog-track">
                            ...
                            <div class="ws-tier-status-box" style="padding:14px; display:flex; flex-direction:column; justify-content:space-between; background:linear-gradient(135deg, #FFFFFF 0%, #FEFAF0 100%); border:1.5px solid rgba(212,175,55,0.45); border-radius:12px;">
                                <div>
                                    ...
                                    <p id="statsMilestoneDesc" style="font-size:0.73rem; color:#57534E; line-height:1.35; margin:0;">
                                        Complete <strong><?= $tierRemaining ?> more orders</strong> to automatically unlock <strong>Tier 2: Silver Retailer</strong> with an extra +3% margin rebate!
                                    </p>
                                </div>
            ...
            <section class="ws-tab-pane" id="tabPaneReports">
                ...
                <div class="ws-report-kpis-grid">
                    ...
                    <div class="ws-kpi-box">
                        ...
                        <div class="ws-kpi-sub"><?= $totalOrders ?> Retail Consignments</div>
                    </div>
                    <div class="ws-kpi-box">
                        <div class="ws-kpi-label">Avg. Consignment Value</div>
            ...
            <section class="ws-tab-pane" id="tabPaneSupport">
                <div class="ws-card">
                    <div class="ws-card-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                        ...
                        <a href="https://api.whatsapp.com/send?phone=917046363528&text=Hi%20DT%20Brand%27s%2C%20I%20am%20a%20registered%20Retailer%20and%20require%20urgent%20support" target="_blank" class="ws-btn ws-btn-wa ws-btn-sm" style="display:inline-flex; align-items:center; gap:6px; font-weight:700;">
        ...
        <div class="ws-modal-box" style="max-width: 620px;">
            <div class="ws-modal-header">
                <h3 class="ws-modal-title">
                    ...
                    <span id="modalOrderTitle">Retail Order Details</span>
                </h3>
        ...
        <div class="ws-modal-box" style="max-width: 440px;">
            <div class="ws-gate-box">
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    <a href="/account.php?tab=login&role=retailer" class="ws-btn ws-btn-primary" style="text-decoration:none; justify-content:center;">
                        <span>Sign In to Retailer Portal</span>
                    ...
                    <a href="https://api.whatsapp.com/send?phone=917046363528&text=Namaste+DT+Brands%2C+I+want+to+activate+my+Retailer+B2B+Partner+Account" target="_blank" class="ws-btn ws-btn-emerald" style="text-decoration:none; justify-content:center;">
                    ...
                    <a href="/account.php?tab=register&role=retailer" class="ws-btn ws-btn-secondary" style="text-decoration:none; justify-content:center;">
    ...
    window.b2bUser = <?= json_encode([
        ...
        'companyName' => !empty($dbUser['company_name']) ? $dbUser['company_name'] : (!empty($dbUser['business_name']) ? $dbUser['business_name'] : (!empty($currentUser['companyName']) ? $currentUser['companyName'] : $activeUserName)),
        'company_name' => !empty($dbUser['company_name']) ? $dbUser['company_name'] : (!empty($dbUser['business_name']) ? $dbUser['business_name'] : (!empty($currentUser['companyName']) ? $currentUser['companyName'] : $activeUserName)),
        'business_name' => !empty($dbUser['business_name']) ? $dbUser['business_name'] : (!empty($currentUser['business_name']) ? $currentUser['business_name'] : $activeUserName),
    ...
    ═══════════════════════════════════════════ -->
    <script src="/assets/js/retailer.js?v=<?= time() ?>"></script>
